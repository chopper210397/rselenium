...
data<-read_xls("C:\\Users\\LBarrios\\Downloads\\AVANCE DE VENTAS LANSIER 12.03.22.xls", sheet = "DATA",skip = 4)
...
data2<-data %>%
  select(FUENTE,periodo,NUMERO,FECHA,`RUC/DNI`,ELIMINAR,
         ...
         CANT.,VALOR,DSCVEND,VENDEDOR,ppuni,ppsol,flag,DEPARTAMENTO,DISTRITO...27,PROVINCIA...28)
...
data_maestro<-data_maestro %>% mutate(COMBINACION=paste0(PROVINCIA...28,DISTRITO...27))
...
data_maestro_localidades<-data_maestro_localidades %>%
  select(FUENTE,periodo,NUMERO,FECHA,`RUC/DNI`,ELIMINAR,`NOMBRE COMERCIAL`,TIPO,ARTDES,artdesvalid,CANT.,
         VALOR,DSCVEND,ZONA,VENDEDOR,tipo,equipo,ppuni,ppsol,flag,DEPARTAMENTO,DISTRITO...27,PROVINCIA...28)
